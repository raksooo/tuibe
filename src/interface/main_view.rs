...
pub struct MainViewActions {
    ...
    main_view_sender: flume::Sender<MainViewMsg>,
}
...
impl MainViewActions {
    pub fn close_config_view(&self) {
        self.config_provider_actions
            .handle_result(self.main_view_sender.send(MainViewMsg::CloseConfig));
    }
        ...
        self.config_provider_actions
            .handle_result_async(
                self.main_view_sender
                    .send_async(MainViewMsg::CloseConfig)
                    .await,
            )
            .await;
    }
...
impl MainView {
    ...
    fn listen_main_view_msg(&self, main_view_receiver: flume::Receiver<MainViewMsg>) {
        ...
        tokio::spawn(async move {
            while let Ok(MainViewMsg::CloseConfig) = main_view_receiver.recv_async().await {
                {
